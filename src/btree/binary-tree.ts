import { DebugLogger, LoggerPlugin, Utils } from 'acebase-core';
...
import { TX } from './tx';
import { _compareBinary, _isEqual, _isLess, _isLessOrEqual, _isMore, _isMoreOrEqual, _isNotEqual } from './typesafe-compare';
import { _appendToArray, _checkNewEntryArgs } from './utils';
const { bigintToBytes } = Utils;

// type ReadFunction = (index: number, length: number) => Buffer | Promise<Buffer|number[]>;
...
/**
 * Allow entry values to be temporarily enriched with an `rp` property
 * that allows for faster comparison of record pointers
 */
type EnrichedBinaryBPlusTreeLeafEntryValue = BinaryBPlusTreeLeafEntryValue & { rp: string };

type PerformanceMeasureObject = { [key: string]: number | PerformanceMeasureObject };
...
export class BinaryBPlusTree {
    ...
    static TransactionOperation = BinaryBPlusTreeTransactionOperation;

    private _chunkSize: number;
    private _autoGrow: boolean;
    private debugData: [string, number][];
    private _writeFn: WriteFunction;
    private _readFn: ReadFunction;
    private _originalByteLength?: number;
    private _fst: Array<{ index: number; length: number }>;
    ...
    constructor(init: {
        ...
        if (init.readFn instanceof Array) {
            ...
            if (BPlusTree.debugBinary) {
                this.debugData = data as any as [string, number][];
                data = this.debugData.map(entry => entry instanceof Array ? entry[1] : entry);
            }
            this._readFn = async (i: number, length: number) => {
    ...
    static async test(data: number[], debug: DebugLogger) {
        const tree = new BinaryBPlusTree({ readFn: data, logger: debug });
    ...
    set autoGrow(grow: boolean) {
        ...
        // if (this._autoGrow) {
        //    this.logger.warn('autoGrow enabled for binary tree');
        // }
    }
    ...
    private async _getReader(): Promise<BinaryReader> {
        ...
        //    this.logger.warn(`Warning: tree "${this.id}" is read-only because it contains small ptrs. it needs to be rebuilt`);
        // }
    ...
    private async _readChild(reader: BinaryReader): Promise<BinaryBPlusTreeNodeInfo> {
    ...
    private _getLeaf(leafInfo: BinaryBPlusTreeNodeInfo, reader: BinaryReader, options?: { stats?: boolean }): BinaryBPlusTreeLeaf {
        if (!this.info) { throw new Error('Tree info has not been read'); }
        ...
        const readEntryValue = (bytes: Buffer | number[], index: number) => {
            ...
            // metadata:
            const metadata: LeafEntryMetaData = this.info.hasMetadata ? {} : undefined;
            ...
            if (options && options.stats) {
                ...
                if (hasExtData) {
                    ...
                    entry.extData = {
                        ...
                        _length: -1,
                        _freeBytes: -1,
                        _values: null as BinaryBPlusTreeLeafEntryValue[],
                        ...
                        async loadValues(existingLock: ThreadSafeLock = null) {
                            // load all values
                            // reader = reader.clone();
                            const self = (this as IBinaryBPlusTreeLeafEntryExtData);
                            const lock = await self.loadHeader(existingLock || true) as ThreadSafeLock;
                            await reader.go(self.index + self._headerLength);
                            const extData = await reader.get(self._length - self._freeBytes);
                            self._values = [] as BinaryBPlusTreeLeafEntryValue[];
                            let index = 0;
                        ...
                        async loadHeader(lockOptions?: true | ThreadSafeLock) {
                            const self = (this as IBinaryBPlusTreeLeafEntryExtData);
                        ...
                        loadFromExtData(allExtData: Buffer) {
                            const self = (this as IBinaryBPlusTreeLeafEntryExtData);
                        ...
                        async addValue(recordPointer: LeafEntryRecordPointer, metadata: LeafEntryMetaData) {
                            // add value to this entry's extData block.

                            const self = (this as IBinaryBPlusTreeLeafEntryExtData);
                            const lock = await self.loadHeader(true) as ThreadSafeLock;
                            ...
                            if (extValueData.length > self._freeBytes) {
                                ...
                                const requiredSpace = (() => {
                                    ...
                                    return {
                                        bytes: netNewLength + self._headerLength, // + 8 header bytes
                                        length: netNewLength,
                                ...
                                if (requiredSpace.bytes > leaf.extData.freeBytes) {
                                    ...
                                    //this.logger.info(`addValue :: moving ext_block from index ${oldIndex} to ${entry.extData.index}, leaf's ext_data_free_bytes reduces from ${oldLeafExtFreeBytes} to ${leaf.extData.freeBytes} bytes`)
                                    extBlockMoves = true;
                                }
                            }
                            else {
                            ...
                            try {
                                //this.logger.info(`TreeWrite:ext_block_length(${entry.extData.length}), ext_block_free_length(${entry.extData.freeBytes})${extBlockMoves ? ', value_list' : ''} :: ${extDataBlock.length} bytes at index ${displayIndex(self.index)}: ${displayBytes(extDataBlock.slice(0,4))}, ${displayBytes(extDataBlock.slice(4,8))}${extBlockMoves ? ', [...]' : ''}`);
                                //this.logger.info(`TreeWrite:value_list_length(${self.totalValues + 1}) :: ${valueListLengthData.length} bytes at index ${displayIndex(self._listLengthIndex)}: ${displayBytes(valueListLengthData)}`);
                                const promises = [
                                ...
                                if (extBlockMoves) {
                                    ...
                                    //this.logger.info(`TreeWrite:ext_data_ptr(${extDataOffset}) :: ${writeBytes.length} bytes at index ${displayIndex(self._listLengthIndex + 4)}: ${displayBytes(writeBytes)}`);
                                    ...
                                    //this.logger.info(`TreeWrite:ext_free_byte_length(${leaf.extData.freeBytes}) :: ${writeBytes.length} bytes at index ${displayIndex(leafExtFreeBytesIndex)}: ${displayBytes(writeBytes)}`);
                                    p = tree._writeFn(writeBytes, leafExtFreeBytesIndex);
                                    promises.push(p);
                                }
                                else {
                                    // write new value:
                                    //this.logger.info(`TreeWrite:value :: ${extValueData.length} bytes at index ${displayIndex(newValueIndex)}: ${displayBytes(extValueData)}`);
                                ...
                                // TEST
                                // try {
                                //    this.logger.info(`Values for entry '${entry.key}' updated: ${self.totalValues} values`);
                                //     await tree._testTree();
                                //    this.logger.info(`Successfully added value to entry '${entry.key}'`);
                                // }
                                // catch (err) {
                                //    this.logger.error(`Tree is broken after updating entry '${entry.key}': ${err.message}`);
                                // }
                            }
                            finally {
                                await lock.release();
                            ...
                            // }
                            // catch (err) {
                            //    this.logger.error(`Values are broken after updating entry '${entry.key}': ${err.message}`);
                            // }
                        },

                        async removeValue(recordPointer: LeafEntryRecordPointer) {
                            // remove value
                    ...
                    if (repairMode) {
                        ...
                        if (currentEntryIndex === -1) {
                            ...
                            if (!currentLeaf.parentNode.parentNode) {
                                ...
                                this.logger.warn(`B+Tree repair: no more leafs in tree`);
                                return null;
                            }
                        }

                        const getNextNode = async (currentNode: BinaryBPlusTreeNode): Promise<BinaryBPlusTreeNode> => {
                        ...
                        const getNextNodeEntry = async (currentNode: BinaryBPlusTreeNode, currentIndex: number) => {
                        ...
                        while (currentNode) {
                            ...
                            try {
                                ...
                                this.logger.warn(`B+Tree repair: using leaf for key ${entryKey} at index ${nextLeaf.index}`);
                                return nextLeaf;
                            }
                            catch (err) {
                                this.logger.warn(
                                    `B+Tree repair: failed to load leaf for key ${entryKey} at index ${currentNodeEntry?.ltChildIndex ?? currentNode.gtChildIndex}: ${err.message}. ` +
                        ...
                        // no more nodes
                        this.logger.warn(`B+Tree repair: there are no more leafs to load`);
                        return null;
                    }
                    throw err;
                }
    ...
    private async _writeNode(nodeInfo: BinaryBPlusTreeNode): Promise<unknown> {
     ...
     * @returns
     */
    async _threadSafe<ReturnType = any>(mode: 'exclusive' | 'shared', fn: () => ReturnType | Promise<ReturnType>): Promise<ReturnType> {
        if (!this.id) {
    ...
    async getFirstLeaf(options?: { stats?: boolean }) {
        return this._threadSafe('shared', () => this._getFirstLeaf(options));
    }

    private async _getFirstLeaf(options?: { stats?: boolean }) {
    ...
    async getLastLeaf(options?: { stats?: boolean }) {
        return this._threadSafe('shared', () => this._getLastLeaf(options));
    }

    private async _getLastLeaf(options?: { stats?: boolean }) {
        const reader = await this._getReader();
        let nodeInfo = await this._readChild(reader);
        while (!nodeInfo.isLeaf) {
            const node = this._getNode(nodeInfo, reader);
    ...
    async findLeaf(searchKey: NodeEntryKeyType, options?: { stats?: boolean }) {
        return this._threadSafe('shared', () => this._findLeaf(searchKey, options));
    }

    private async _findLeaf(searchKey: NodeEntryKeyType, options?: { stats?: boolean }) {
    ...
    private _search(op: string | BlacklistingSearchOperator, param: NodeEntryKeyType | NodeEntryKeyType[] | RegExp, include: { entries?: boolean; values?: boolean; keys?: boolean; count?: boolean; filter?: BinaryBPlusTreeLeafEntry[] } = { entries: true, values: false, keys: false, count: false }) {
        ...
        const results = {
            entries: [] as BinaryBPlusTreeLeafEntry[],
            keys: [] as NodeEntryKeyType[],
            keyCount: 0,
            valueCount: 0,
            values: [] as BinaryBPlusTreeLeafEntryValue[], // was not implemented? is include.values used anywhere?
        };

        let blacklistRpTree: BPlusTree;
        ...
        // let totalMatches = 0;
        // let totalAdded = 0;
        const valuePromises = [] as Promise<any>[];
        const emptyValue: LeafEntryRecordPointer = [];

        const add = (entry: BinaryBPlusTreeLeafEntry): Promise<any> | any => {
            ...
            if (filterRecordPointers || op instanceof BlacklistingSearchOperator) {
                ...
                entry.values.forEach(val => {
                    (val as EnrichedBinaryBPlusTreeLeafEntryValue).rp = String.fromCharCode(...val.recordPointer);
                });
            }
            if (filterRecordPointers) {
                ...
                // Using string comparison:
                const values = entry.values.filter(val => filterRecordPointers.includes((val as EnrichedBinaryBPlusTreeLeafEntryValue).rp));
            ...
            if (op instanceof BlacklistingSearchOperator) {
                ...
                entry.values = entry.values.filter(val => {
                    return blacklistRpTree.find((val as EnrichedBinaryBPlusTreeLeafEntryValue).rp) === null;
                });
                ...
                if (blacklistValues instanceof Array) {
                    ...
                    blacklistValues.forEach(val => {
                        blacklistRpTree.add((val as EnrichedBinaryBPlusTreeLeafEntryValue).rp, emptyValue);
                    });
                    ...
                    if (include.values) {
                        ...
                        for (let i = 0; i < results.values.length; i++) {
                            ...
                            if (blacklistRpTree.find((val as EnrichedBinaryBPlusTreeLeafEntryValue).rp)) {
                ...
                for (let i = 0; i < entry.values.length; i++) {
                    const currentValue = entry.values[i] as EnrichedBinaryBPlusTreeLeafEntryValue;
                    let remove = false;
                    if (include.values) {
                        // TODO: change to: remove = results.values.includes
                        const index = results.values.findIndex(val => (val as EnrichedBinaryBPlusTreeLeafEntryValue).rp === currentValue.rp);
                        remove = index >= 0;
                    }
                        ...
                        for (let j = 0; j < results.entries.length; j++) {
                            ...
                            const index = entry.values.findIndex(val => (val as EnrichedBinaryBPlusTreeLeafEntryValue).rp === currentValue.rp);
                            remove = index >= 0;
                            if (remove) { break; }
                        }
                    }
        ...
        // const ret = () => {
        //     const t2 = Date.now();
        //    this.logger.info(`tree.search [${op} ${param}] took ${t2-t1}ms, matched ${totalMatches} values, returning ${totalAdded} values in ${results.entries.length} entries`);
        //     return results;
        // };
        const ret = () => {
        ...
        if (op instanceof BlacklistingSearchOperator) {
            ...
            const processLeaf = (leaf: BinaryBPlusTreeLeaf): ReturnType<typeof ret> => {
                for (let i = 0; i < leaf.entries.length; i++) {
                    const entry = leaf.entries[i];
            ...
            return this._findLeaf(param as NodeEntryKeyType, getLeafOptions)
                .then(processLeaf);
        }
        else if (op === '==') {
            return this._findLeaf(param as NodeEntryKeyType, getLeafOptions)
            ...
            const processLeaf = (leaf: BinaryBPlusTreeLeaf): ReturnType<typeof ret> => {
                for (let i = 0; i < leaf.entries.length; i++) {
                    const entry = leaf.entries[i];
                    if (!re.test(entry.key.toString())) { add(entry); }
                }
                if (leaf.hasNext) {
                    return leaf.getNext()
                        .then(processLeaf);
    ...
    async _growTree(bytesNeeded: number) {
        if (!this.info) { throw new NoTreeInfoError(); }
        if (!this._autoGrow) {
     ...
     */
    async setAllocationBytes(buffer: Uint8Array, byteLength: number, freeSpace: number) {
        // byte_length:
    ...
    async _registerFreeSpace(index: number, length: number) {
        if (!this.info) { throw new NoTreeInfoError(); }
        ...
        if (index + length === this.info.byteLength - this.info.freeSpace) {
            ...
            //this.logger.info(`Freeing ${length} bytes from index ${index} (at end of file)`);
            ...
            //this.logger.info(`Freeing ${length} bytes from index ${index} to ${index+length}`);
    ...
    async _claimFreeSpace(bytesRequired: number) {
        if (!this.info) { throw new NoTreeInfoError(); }
    ...
    async _requestFreeSpace(bytesRequired: number) {
        if (!this.info) { throw new NoTreeInfoError(); }
     ...
     */
    async _rebuildLeaf(leaf: BinaryBPlusTreeLeaf, options: {
        growData?: boolean;
        growExtData?: boolean;
        ...
        try {
            let allocated: { index: number; length: number; };
            if (oneLeafTree) {
            ...
            //this.logger.info(`Rebuilding leaf for entries "${leaf.entries[0].key}" to "${leaf.entries[leaf.entries.length-1].key}"`);
            ...
            tx.queue({
                ...
                action: async () => {
                    ...
                    //this.logger.info(`new leaf for entries "${newLeaf.entries[0].key}" to "${newLeaf.entries.slice(-1)[0].key}" was written successfully at index ${newLeaf.index} (used to be at ${leaf.index})`);
    ...
    async _splitNode(node: BinaryBPlusTreeNode, options: { keepEntries?: number; cancelCallback?: () => unknown} = { keepEntries: 0, cancelCallback: null }): Promise<{ node1: BinaryBPlusTreeNode, node2: BinaryBPlusTreeNode }> {
        ...
        try {
            ...
            //this.logger.info(`Splitting node "${node.entries[0].key}" to "${node.entries.slice(-1)[0].key}", cutting at "${movingEntries[0].key}"`);
            ...
            // });
            //this.logger.info(`Creating new node for ${movingEntries.length} entries`);
    ...
    async _splitLeaf(leaf: BinaryBPlusTreeLeaf, options: { nextLeaf?: BinaryBPlusTreeLeaf; keepEntries?: number; cancelCallback?: () => unknown } = { nextLeaf: null, keepEntries: 0, cancelCallback: null }) {
        ...
        try {
            ...
            //this.logger.info(`Splitting leaf "${leaf.entries[0].key}" to "${leaf.entries.slice(-1)[0].key}", cutting at "${movingEntries[0].key}"`);
            ...
            //this.logger.info(`Creating new leaf for ${movingEntries.length} entries`);
    ...
    //     }
    //    this.logger.warn(`TREE TEST: testing ${keys.length} keys`);
    //     //this.logger.warn(keys);
    ...
    //     }
    //    this.logger.warn(`TREE TEST: testing ext_data`);
    ...
    //     }
    //    this.logger.warn(`TREE TEST SUCCESSFUL`);
    // }
    ...
    async _add(key: NodeEntryKeyType, recordPointer: LeafEntryRecordPointer, metadata?: LeafEntryMetaData) {
                ...
                if (entryIndex >= 0) {
                    ...
                    if (entry.extData) {
                        try {
                            ...
                            // and needs to grow
                            //this.logger.info(`Leaf rebuild necessary - unable to add value to key "${key}": ${err.message}`);
                            ...
                            const rebuildOptions = {
                                ...
                                applyChanges: (leaf: BinaryBPlusTreeLeaf) => {
        ...
        //    this.logger.warn(`TESTING leaf adjustment after adding "${key}". Remove code when all is well!`);
    ...
    async _process(operations: BinaryBPlusTreeTransactionOperation[]) {
        if (!this.info) {
            await this._loadInfo();
        }
        if (!this.info.isUnique) {
        ...
        try {
            ...
            const saveLeaf = async () => {
                ...
                if (leaf.entries.length > this.info.entriesPerNode) {
                    ...
                    //this.logger.info('*process _splitLeaf');
                    ...
                    // Remove leaf
                    //this.logger.info('*process _removeLeaf');
            ...
            while (operations.length > 0) {
                ...
                if (type === 'remove') {
                    ...
                    if (!entry) {
                        ...
                        undo.push(() => {
                            //this.logger.info(`Undo remove ${entry.key}`);
        ...
        //            this.logger.info(debugThrownError);
        //             debugger;
        //         }
        //     }
        // }
    ...
    async remove(key: NodeEntryKeyType, recordPointer?: LeafEntryRecordPointer) {
        return this._threadSafe('exclusive', () => this._remove(key, recordPointer));
    }

    async _remove(key: NodeEntryKeyType, recordPointer?: LeafEntryRecordPointer) {
     ...
     */
    async _removeLeaf(leaf: BinaryBPlusTreeLeaf) {
        try {
    ...
    /**
     * Removes an empty leaf whose parent node has exactly 1 entry (removing that entry would
     * leave the parent with 0 entries, which is invalid). For non-root parents this is handled
     * efficiently by updating the grandparent to bypass the now-empty parent node — no full
     * tree rebuild required. For root parents the tree depth must decrease, so a safe in-memory
     * rebuild is used (cheap because the tree is very small at that point).
     */
    async _removeLeafWithEmptyParent(leaf: BinaryBPlusTreeLeaf) {
        try {
            assert(leaf.entries.length === 0, 'Leaf must be empty');
            assert(leaf.parentNode, 'Leaf must have a parent node');
            assert(leaf.parentNode.entries.length === 1, 'Parent node must have exactly 1 entry');

            const freedLeafBytes = leaf.length + leaf.extData.length;

            // ------------------------------------------------------------------
            // Step 1: Update the parent node in memory — remove the leaf reference.
            //         After this, parent has 0 entries and a single gtChild.
            // ------------------------------------------------------------------
            const parentOriginalEntries = leaf.parentNode.entries.map(e => {
                const copy = new BinaryBPlusTreeNodeEntry(e.key);
                copy.ltChildIndex = e.ltChildIndex;
                return copy;
            });
            const parentOriginalGtChildIndex = leaf.parentNode.gtChildIndex;

            if (leaf.parentEntry) {
                leaf.parentNode.entries.splice(leaf.parentNode.entries.indexOf(leaf.parentEntry), 1);
            }
            else {
                // Leaf is the gtChild — promote the only entry's ltChild to gtChild.
                const lastEntry = leaf.parentNode.entries.splice(-1)[0];
                leaf.parentNode.gtChildIndex = lastEntry.ltChildIndex;
            }
            const parentGtChildIndex = leaf.parentNode.gtChildIndex; // the one remaining child

            // ------------------------------------------------------------------
            // Step 2a: Non-root parent — update grandparent to bypass the empty
            //          parent node, then free both the leaf and the parent.
            // ------------------------------------------------------------------
            if (leaf.parentNode.parentNode) {
                const grandParentOriginalEntries = leaf.parentNode.parentNode.entries.map(e => {
                    const copy = new BinaryBPlusTreeNodeEntry(e.key);
                    copy.ltChildIndex = e.ltChildIndex;
                    return copy;
                });
                const grandParentOriginalGtChildIndex = leaf.parentNode.parentNode.gtChildIndex;

                // Redirect grandparent's pointer from parent → parent's only remaining child.
                if (leaf.parentNode.parentEntry) {
                    leaf.parentNode.parentEntry.ltChildIndex = parentGtChildIndex;
                }
                else {
                    leaf.parentNode.parentNode.gtChildIndex = parentGtChildIndex;
                }

                const tx = new TX();

                // Patch prev leaf's next_leaf_ptr to skip this leaf.
                if (leaf.hasPrevious) {
                    const prevLeaf = {
                        nextPointerIndex: leaf.prevLeafIndex + BinaryBPlusTreeLeaf.nextLeafPtrIndex,
                        oldOffset: BinaryBPlusTreeLeaf.getNextLeafOffset(leaf.prevLeafIndex, leaf.index),
                    ...
                    tx.queue({
                        name: 'prev leaf next_leaf_ptr',
                        action: async () => this._writeFn(writeSignedOffset([], 0, prevLeaf.newOffset, true), prevLeaf.nextPointerIndex),
                        rollback: async () => this._writeFn(writeSignedOffset([], 0, prevLeaf.oldOffset, true), prevLeaf.nextPointerIndex),
                    });
                }

                // Patch next leaf's prev_leaf_ptr to skip this leaf.
                if (leaf.hasNext) {
                    const nextLeaf = {
                        prevPointerIndex: leaf.nextLeafIndex + BinaryBPlusTreeLeaf.prevLeafPtrIndex,
                        oldOffset: BinaryBPlusTreeLeaf.getPrevLeafOffset(leaf.nextLeafIndex, leaf.index),
                    ...
                    tx.queue({
                        name: 'next leaf prev_leaf_ptr',
                        action: async () => this._writeFn(writeSignedOffset([], 0, nextLeaf.newOffset, true), nextLeaf.prevPointerIndex),
                        rollback: async () => this._writeFn(writeSignedOffset([], 0, nextLeaf.oldOffset, true), nextLeaf.prevPointerIndex),
                    });
                }

                // Write the grandparent with its updated pointer.
                tx.queue({
                    name: 'grandparent node',
                    action: async () => this._writeNode(leaf.parentNode.parentNode),
                    rollback: async () => {
                        leaf.parentNode.parentNode.entries = grandParentOriginalEntries;
                        leaf.parentNode.parentNode.gtChildIndex = grandParentOriginalGtChildIndex;
                        leaf.parentNode.entries = parentOriginalEntries;
                        leaf.parentNode.gtChildIndex = parentOriginalGtChildIndex;
                        return this._writeNode(leaf.parentNode.parentNode);
                    },
                });

                await tx.execute(true);

                // Register freed space for both the removed leaf and the now-bypassed parent node.
                await this._registerFreeSpace(leaf.index, freedLeafBytes);
                await this._registerFreeSpace(leaf.parentNode.index, leaf.parentNode.length);
                await this._loadInfo(); // Ensure this.info reflects the updated tree header
            }
            else {
                // ------------------------------------------------------------------
                // Step 2b: Root parent — tree depth must decrease by one level.
                //          The root's only remaining child becomes the new root.
                //          Write the empty leaf first, then do a safe in-memory rebuild
                //          (cheap: the tree is very small when it reaches this state).
                // ------------------------------------------------------------------
                await this._writeLeaf(leaf);
                const treeBuffer = new Uint8ArrayBuilder();
                await this._rebuild(BinaryWriter.forUint8ArrayBuilder(treeBuffer), {
            ...
            throw new DetailedError('remove-leaf-empty-parent-failed', `Failed to remove leaf with empty parent: ${err.message}`, err);
        }
    }

    async update(key: NodeEntryKeyType, newRecordPointer: LeafEntryRecordPointer, currentRecordPointer?: LeafEntryRecordPointer, newMetadata?: LeafEntryMetaData) {
        return this._threadSafe('exclusive', () => this._update(key, newRecordPointer, currentRecordPointer, newMetadata));
    }

    async _update(key: NodeEntryKeyType, newRecordPointer: LeafEntryRecordPointer, currentRecordPointer?: LeafEntryRecordPointer, newMetadata?: LeafEntryMetaData) {
        try {
            // key = _normalizeKey(key); // if (_isIntString(key)) { key = parseInt(key); }
     ...
     */
    async transaction(operations: BinaryBPlusTreeTransactionOperation[]) {
        return this._threadSafe('exclusive', () => this._transaction(operations));
    }

    async _transaction(operations: BinaryBPlusTreeTransactionOperation[]) {
        if (!this.info) {
     ...
     */
    async toTreeBuilder(fillFactor: number) {
     ...
     * @returns Promise that resolves with a BPlusTreeBuilder
     */
    async _toTreeBuilder(fillFactor: number) {
    ...
    async rebuild(writer: BinaryWriter, options?: {
        ...
        treeStatistics?: Partial<{ byteLength: number, totalEntries: number, totalValues: number, totalLeafs: number, depth: number, entriesPerNode: number }>;
        repairMode?: boolean;
    }) {
        return this._threadSafe('exclusive', () => this._rebuild(writer, options));
    }

    async _rebuild(writer: BinaryWriter, options: {
        allocatedBytes?: number;
        fillFactor?: number;
        keepFreeSpace?: boolean;
        increaseMaxEntries?: boolean;
        ...
        const perf: PerformanceMeasureObject = {};
        const mark = (name: string) => {
            const keys = name.split('.');
            const key = keys.pop();
            const target = keys.reduce((t, key) => key in t ? t[key] : (t[key] = {}), perf) as PerformanceMeasureObject;
            target[key] = Date.now(); // performance.mark(name);
        };
        const measure = (mark1: string, mark2: string) => {
        ...
        const getKeySize = (key: NodeEntryKeyType) => {
        ...
        // let leafsSeen = 0;
        //this.logger.info(`[${Date.toString()}] Starting tree rebuild`);
        try {
            const getLeafStartKeys = async (entriesPerLeaf: number) => {
                ...
                while (leaf) {
                    ...
                    // leafsSeen++;
                    //this.logger.info(`Processing leaf with ${leaf.entries.length} entries, total=${totalEntries}`);
                    ...
                    //this.logger.info(`Processed ${leafsSeen} leafs in source tree`);
                    leaf = leaf.getNext ? await leaf.getNext(options.repairMode) : null;
                }
            ...
            let lastLeaf: BinaryBPlusTreeLeaf = null;
            let getEntryCalls = 1;
            /**
             * Gets next leaf's entries
             * @param n unused
             */
            const getEntries = async (n: number): Promise<BinaryBPlusTreeLeafEntry[]> => {
            ...
            if (perf) {
                // inspect perf here
                this.logger.trace(`[perf] tree rebuild took ${measure('start', 'end')}ms`);
                this.logger.trace(`[perf] getLeafStartKeys: ${measure('getLeafStartKeys.start', 'getLeafStartKeys.end')}ms`);
                this.logger.trace(`[perf] getEntries: ${measure('getEntries.first', 'getEntries.last')}ms`);
                this.logger.trace(`[perf] tree.create: ${measure('tree.createStart', 'tree.createEnd')}ms`);
            }
        }
    }
    ...
    static async create(options: {
        ...
        try {
            ...
            type ChildLevelNodeEntry = {
                entries: {
                    key: NodeEntryKeyType,
                    ltChildIndex: number
                }[];
                gtChildIndex: number;
                gtMaxKey: number | null;
                index?: number;
                byteLength?: number;
            };
            let childLevelNodes: Array<NodeEntryKeyType | ChildLevelNodeEntry> = leafStartKeys;
            const levels: Array<ChildLevelNodeEntry[]> = [];
            while (childLevelNodes.length > 1) {
                // Create another level
                childLevelNodes = childLevelNodes.reduce((nodes, child: NodeEntryKeyType | ChildLevelNodeEntry, index, arr) => {
                ...
                levels.push(childLevelNodes as ChildLevelNodeEntry[]);
            }
            ...
            // Write all leafs
            const newLeafEntries: BinaryBPlusTreeLeafEntry[] = [];
            let prevIndex = 0;
            let currentLeafIndex = 0;
            let totalWrittenEntries = 0;
            const writeLeaf = async (entries: BinaryBPlusTreeLeafEntry[]) => {
                let emptyLeaf = false;
                ...
                // logger.debug(`Writing leaf with ${entries.length} entries at index ${index}, keys range: ["${entries[0].key}", "${entries[entries.length-1].key}"]`)
    ...
    static createFromEntryStream(reader: BinaryReader, writer: BinaryWriter, options: {
        ...
        const getEntries = async (n: number) => {
            // read n entries
            const entries = [];
            ...
            while (true) {
                try {
                    ...
                    const entry = new BinaryBPlusTreeLeafEntry(
                        k.value,
                        [],
                    );
                    ...
                    for(let i = 0; i < totalValues; i++) {
                        ...
                        const metadata: LeafEntryMetaData = {};
                        ...
                        const value = new BinaryBPlusTreeLeafEntryValue(
                            recordPointer,
                            metadata,
                        );
