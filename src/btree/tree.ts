...
export class BPlusTree {
     ...
     */
    public root: BPlusTreeLeaf | BPlusTreeNode;
     ...
     */
    constructor(public maxEntriesPerNode: number, public uniqueKeys: boolean, public metadataKeys: string[] = []) {
     ...
     */
    add(key: NodeEntryKeyType, value: number[]|Uint8Array, metadata?: LeafEntryMetaData) {
    ...
    findLeaf(key: NodeEntryKeyType): BPlusTreeLeaf {
        ...
        const findLeaf = (node: BPlusTreeNode | BPlusTreeLeaf): BPlusTreeLeaf => {
    ...
    search(op: string, val: NodeEntryValueType | NodeEntryValueType[]) {
        ...
        const add = (entry: BPlusTreeLeafEntry) => {
            const obj: typeof results['0'] = { key: entry.key };
        ...
        if (['<','<='].includes(op)) {
            let leaf = this.findLeaf(val as NodeEntryValueType);
            while(leaf) {
            ...
            const leaf = this.findLeaf(val as NodeEntryValueType);
            ...
            const sorted = (val as NodeEntryValueType[]).slice().sort();
            ...
            // Full index scan needed
            const keys = val as NodeEntryValueType[];
            let leaf = this.firstLeaf();
            while(leaf) {
                for (let i = 0; i < leaf.entries.length; i++) {
                    const entry = leaf.entries[i];
    ...
    static addBinaryDebugString(str: string, byte: number) {
        if (this.debugBinary) {
            return [str, byte];
        }
        else {
    ...
    static getBinaryKeyData(key: NodeEntryKeyType) {
    ...
    static get typeSafeComparison() {
        return {
            isMore(val1: unknown, val2: unknown) { return _isMore(val1, val2); },
            isMoreOrEqual(val1: unknown, val2: unknown) { return _isMoreOrEqual(val1, val2); },
            isLess(val1: unknown, val2: unknown) { return _isLess(val1, val2); },
            isLessOrEqual(val1: unknown, val2: unknown) { return _isLessOrEqual(val1, val2); },
            isEqual(val1: unknown, val2: unknown) { return _isEqual(val1, val2); },
            isNotEqual(val1: unknown, val2: unknown) { return _isNotEqual(val1, val2); },
        };
    }
}
