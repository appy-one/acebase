...
const _currentNodeLookups = new Map();
...
class Node {
     ...
     */
    static getInfo(storage, path, options = { tid: undefined }) {
        if (!options) {
        ...
        if (_currentNodeLookups.has(path)) {
            // Bind to current lookup in progress, so we don't start another lookup
            // const lookupHooks = _currentNodeLookups.get(path);
            return new Promise((resolve, reject) => {
                const lookupHooks = _currentNodeLookups.get(path);
                if (!lookupHooks) { 
                    // It has resolved in the mean time...
                    return Node.getInfo(storage, path, tid);
                }
                // Bind to it, resolve later
                lookupHooks.push({ resolve, reject });
            })
        }
        ...
        .then(parentInfo => {
            ...
            if (!parentInfo.exists || [VALUE_TYPES.OBJECT, VALUE_TYPES.ARRAY].indexOf(parentInfo.valueType) < 0 || !parentInfo.address) {
                ...
                storage.nodeCache.update(childInfo);
                return childInfo;
            }

            const reader = new NodeReader(storage, parentInfo.address, lock, true);
            return reader.getChildInfo(pathInfo.key);
        })
        .then(childInfo => {
            lock.release(`Node.getInfo: done with path "/${parentPath}"`);
            // NodeCache.update(childInfo); // Don't have to, nodeReader will have done it already
            return childInfo;
        })
        .then(nodeInfo => {
            const otherLookups = _currentNodeLookups.get(path);
            _currentNodeLookups.delete(path);
            otherLookups && otherLookups.forEach(lookup => lookup.resolve(nodeInfo));
            return nodeInfo;
        });
    }

    /**
     * @deprecated Use Node.getInfo
     * @param {Storage} storage 
     * @param {string} path 
     * @returns {Promise<NodeInfo>} promise that resolves with info about the node
     */
    ...
    static update(storage, path, value, options = { merge: true, tid: undefined, _internal: false }) {
        ...
        .then(eventNodeInfo => {
            ...
            if (!hasValueSubscribers && options.merge === true) {
                ...
                storage.indexes.getAll(path, false).forEach(index => {
            ...
            let reader = new NodeReader(storage, eventNodeInfo.address, lock, false); //, eventDataLock);
        ...
        .then(result => {
            ...
            .then(parentUpdated => {
                if (parentUpdated && pathInfo.parentPath !== '') {
                    console.assert(storage.nodeCache._cache.has(pathInfo.parentPath), 'Not cached?!!');
                }
                lock && lock.release();
                ...
                if (options._internal !== true) {    
                    ...
                    if (eventSubscriptions.length > 0 || indexes.length > 0) {
                        ...
                        const runIndexUpdate = (index, path, oldValue, newValue) => {
                            ...
                            // Disabled conditional execution of handleRecordUpdate
                            // Reason: if the index includes other keys, it must also be updated if one
                            // of those keys changed. This is all checked by handleRecordUpdate
    ...
    static getValue(storage, path, options = { tid: undefined, include: undefined, exclude: undefined, child_objects: true }) {
        ...
        .then(nodeInfo => {
            ...
            if (nodeInfo.address) {
                let reader = new NodeReader(storage, nodeInfo.address, lock, true);
