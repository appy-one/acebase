...
export class MSSQLStorage extends Storage {
    ...
    private mssql: any;
    private _db: any;
    private rootRecord: MSSQLNodeInfo;
    ...
    private _executeRequest(request: any, sql: string, params: any) {
    ...
    private async _get(sql: string, params?: any) {
    ...
    private async _getOne(sql: string, params?: any) {
        const rows = await this._get(sql, params);
        return rows[0] as any;
    }

    private async _exec(sql: string, params?: any) {
        const request = new this.mssql.Request(this._db);
        const result = await this._executeRequest(request, sql, params);
     ...
     */
    private _each(sql: string, params: any = {}, callback: (row: any) => boolean) {
        // stream
        const request = new this.mssql.Request(this._db);
        request.stream = true;

        return new Promise<{ rows: number; canceled: boolean }>((resolve, reject) => {
            let totalRows = 0;
            let canceled = false;
     ...
     */
    _deleteNode(path: string, options = { transaction: null as ReturnType<MSSQLStorage['_createTransaction']> }) {
    ...
    getChildren(
        ...
        const start = async () => {
            ...
            try {
                const row: any = await this._getOne(`SELECT type, json_value, revision, revision_nr, created, modified FROM nodes WHERE path = @path`, { path: path });
