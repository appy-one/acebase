...
export class CustomStorageHelpers {
     ...
     */
    static ChildPathsSql(path: string, columnName = 'path') {
        const where = path === ''
     ...
     */
    static ChildPathsRegex(path: string) {
     ...
     * @returns Returns the SQL where clause
     */
    static DescendantPathsSql(path: string, columnName = 'path') {
        const where = path === ''
            ? `${columnName} <> ''`
     ...
     * @returns Returns regular expression to test paths with
     */
    static DescendantPathsRegex(path: string) {
        return new RegExp(`^${path}(?:/[^/[]+|\\[[0-9]+\\])`);
    }
