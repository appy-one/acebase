...
export class TxDetailedError extends DetailedError {
    ...
    rollbackErrors: Array<DetailedError|Error> = null;
...
export class TX {
    ...
    private _queue: Array<{ name: string; action: (prevResult?: any) => any; rollback: () => void; state: 'idle'|'success'|'failed'; error: Error; result?: any }>;
    private _rollbackSteps: Array<() => void>;
     ...
     */
    queue(step: { name?: string; action: (prevResult?: any) => any; rollback: () => any }) {
        this._queue.push({
    ...
    async execute(parallel = true) {
        ...
        const executeStepAction = async (step: typeof this._queue[0], action: 'action'|'rollback') => {
            try {
