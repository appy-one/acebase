...
describe('Query', () => {
    ...
    beforeAll(async () => {
        ...
        const movies = await readDataSet('movies') as { year: number; rating: number; genres: string[] }[];
...
describe('Query with take/skip multiple sorts', () => {
    ...
    beforeAll(async () => {
        ...
        movies = await readDataSet('movies') as any[];
...
describe('Query on indexed BigInts #141', () => {
    ...
    beforeAll(async () => {
        ...
        const movies = (await readDataSet('movies') as any[]).map(movie => {
            return {
                title: movie.title,
                rating: movie.rating,
                year: movie.year,
