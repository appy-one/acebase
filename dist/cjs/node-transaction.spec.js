...
const node_transaction_1 = require("./node-transaction");
describe('NodeTransaction (beta)', () => {
    const manager = new node_transaction_1.TransactionManager();
    ...
    it('should not allow conflicts', () => {
        ...
        // |    |- john
        // |    |    |- address
        const readValue = (path, filter) => ({ path, intention: node_transaction_1.NodeLockIntention.ReadValue(filter) });
        ...
        const readInfo = (path) => ({ path, intention: node_transaction_1.NodeLockIntention.ReadInfo() });
        const setValue = (path) => ({ path, intention: node_transaction_1.NodeLockIntention.OverwriteNode() });
        ...
        // ReadInfo lock on node ""
        // Deny updating node ""
        const updateValue = (path, keys) => ({ path, intention: node_transaction_1.NodeLockIntention.UpdateNode(keys) });
...
//# sourceMappingURL=node-transaction.spec.js.map