...
class DataIndex {
    ...
    async take(skip, take, options = {}) {
        ...
        const stats = new query_stats_1.IndexQueryStats('take', { skip, take, ascending }, true);
        const idx = await this._getTree('shared');
    ...
    async query(op, val, options = {}) {
        ...
        const stats = new query_stats_1.IndexQueryStats('query', { op, val }, true);
        let entries; // ;
        ...
        results.entryValues = [];
        if (options.filter) {
            const filterStep = new query_stats_1.IndexQueryStats('filter', {
            ...
            for (let i = 0; i < smallestSet.length; i++) {
                ...
                if (match) {
                    ...
                    const result = new query_results_1.IndexQueryResult(recordPointer.key, recordPointer.path, entry.key, metadata);
                    // result.entry = entry;
                    results.push(result);
