...
exports.MSSQLStorage = exports.MSSQLStorageSettings = exports.MSSQLNodeInfo = exports.MSSQLNodeAddress = void 0;
...
class MSSQLNodeAddress extends node_address_1.NodeAddress {
    constructor(containerPath) {
        super(containerPath);
    }
}
exports.MSSQLNodeAddress = MSSQLNodeAddress;
class MSSQLNodeInfo extends node_info_1.NodeInfo {
    constructor(info) {
        super(info);
...
exports.MSSQLNodeInfo = MSSQLNodeInfo;
class MSSQLStorageSettings extends __1.StorageSettings {
    constructor(options) {
        super(options);
        /**
...
class MSSQLStorage extends __1.Storage {
    /**
    ...
    async init() {
        ...
        const exists = await promise_fs_1.pfs.exists(path);
        if (!exists) {
            try {
        ...
        try {
            ...
            this.debug.log(`- Type: MSSQL`.colorize(acebase_core_1.ColorStyle.dim));
            this.debug.log(`- Server: ${this.settings.server}:${this.settings.port}`.colorize(acebase_core_1.ColorStyle.dim));
            this.debug.log(`- Database: ${this.settings.database}`.colorize(acebase_core_1.ColorStyle.dim));
    ...
    _executeRequest(request, sql, params) {
        ...
        Object.keys(params !== null && params !== void 0 ? params : {}).forEach(name => {
        ...
        const start = async () => {
            ...
            try {
                ...
                keys.length > 0 && keys.every(key => {
                    ...
                    const info = new MSSQLNodeInfo(Object.assign(Object.assign(Object.assign({ path: pathInfo.childPath(key) }, (!isArray && { key })), (isArray && { index: parseInt(key) })), { type: child.type, address: null, exists: true, value: child.value, revision: row.revision, revision_nr: row.revision_nr, created: parseInt(row.created), modified: parseInt(row.modified) }));
                ...
                await this._each(q, null, row => {
                    ...
                    const info = new MSSQLNodeInfo(Object.assign(Object.assign(Object.assign({ path: row.path, type: row.type }, (!isArray && { key: key })), (isArray && { index: key })), { address: new MSSQLNodeAddress(row.path), exists: true, value: null, revision: row.revision, revision_nr: row.revision_nr, created: parseInt(row.created), modified: parseInt(row.modified) }));
                    canceled = callback(info) === false;
    ...
    async getNode(path, options = { child_objects: true }) {
        ...
        try {
            ...
            this.debug.log(`Read node "/${path}" and ${filtered ? '(filtered) ' : ''}children from ${rows.length} records`.colorize(acebase_core_1.ColorStyle.magenta));
